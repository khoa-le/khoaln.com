...
We develop Magento Ecommerce for the client that help them Ecommerce website also Order management. We have a lot of
customize admin page to help them more efficient.
...
Now let's start learn something essential:
...
So the just call the function `refresh` from `Grid` class and update back to the `sales_order_grid` table.
