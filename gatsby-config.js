      ...
      options: {
        feeds: [
          getBlogFeed({
            ...
            blogUrl: 'https://khoaln.com/blog',
            output: '/blog/rss.xml',
            title: "Khoa Le's Blog RSS Feed",
          }),
          getBlogFeed({
            filePathRegex: `//content/writing-blog//`,
            blogUrl: 'https://khoaln.com/writing/blog',
            output: '/writing/blog/rss.xml',
            title: `Khoa's Writing Blog RSS Feed`,
          }),
        ],
      },
    },
  ...
  return {
    serialize: ({query: {allMdx}}) => {
      ...
      return allMdx.nodes.map(node => {
        const url = `${siteUrl}/${stripSlash(node.fields.slug)}`
          ...
          custom_elements: [
            {
              'content:encoded': `<div style="width: 100%; margin: 0 auto; max-width: 800px; padding: 40px 40px;">
                  <p>
                    I've posted a new article <em>"${node.frontmatter.title}"</em> and you can <a href="${url}">read it online</a>.
                    <br>
                    ${node.fields.plainTextDescription}
                    <br>
